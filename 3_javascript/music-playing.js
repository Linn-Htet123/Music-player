     ...
     let song = playList[i];
      ...
      if (song.classList.contains("playlist-song-background")) {
         ...
         if(nextSong == null){
            ...
            musicData.map((data)=>{
               if(song.id == data.id){
                  let finalSong = playList[playList.length -1]
                  audioTag.src =data.songSrc;
                  audioTag.play();
                  song.classList.add("playlist-song-background");
                  finalSong.classList.remove("playlist-song-background");
               }
            })
            nextSong = song.nextSibling.nextSibling;
